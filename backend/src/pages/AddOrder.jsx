...
async function handlePartChange(value) {
  if (value === "add_new_part") {
    ...
    if (newPart && newPart.trim() !== "") {
      try {
        await axios.post("http://localhost:5000/parts", { name: newPart.trim() });
        await fetchParts(); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://localhost:5000/orders/orders?firstName=${encodeURIComponent(
          firstName
        )}`,
        payload
      );
  ...
  return (
    <div className="flex">
      ...
      <input
        ...
        onChange={onChange}
        disabled={disabled}
        className={`w-full p-2 border border-gray-300 bg-white/20 text-white placeholder-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 ${
