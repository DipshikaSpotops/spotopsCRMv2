...
const LS_PAGE = "poreport_page";
const LS_FILTER = "poreport_filter_v2";
const LS_SORTBY = "poreport_sortBy";
const LS_SORTORDER = "poreport_sortOrder";
const LS_HILITE = "poreport_highlightedOrderNo";
const LS_SEARCH = "poreport_search";
...
function computeRow(order) {
  ...
  let totalPart = 0, totalShip = 0, totalOthers = 0, totalRefunds = 0, totalOverallAfterRefund = 0;
  addl.forEach((info) => {
    ...
    totalPart += part; totalShip += ship; totalOthers += others; totalRefunds += refunds;
...
const POReport = () => {
  ...
  // Fetch ALL pages via RTK Query (always use month/year)
  const normalizedForApi = toMonthYearFilter(activeFilter);
  const queryArgs = useMemo(() => {
    const params = buildParams({
      filter: normalizedForApi,
  ...
  const yardOrders = useMemo(
    () =>
      (allOrders || []).filter(
        (o) => Array.isArray(o.additionalInfo) && o.additionalInfo.length > 0
      ),
    [allOrders]
  );
  ...
  const maxYardsOnPage = useMemo(
    () =>
      pageRows.reduce(
        (m, r) => Math.max(m, Array.isArray(r.additionalInfo) ? r.additionalInfo.length : 0),
        0
      ),
    [pageRows]
  );
  ...
  useEffect(() => {
    ...
    if (match) {
      setTimeout(() => {
        const el = document.getElementById(`porep-row-${match._id || match.orderNo}`);
  ...
  useEffect(() => {
    ...
    const writeFilterToSearch = (spIn, filter) => {
      ["start", "end", "month", "year"].forEach((k) => spIn.delete(k));
      if (filter?.start && filter?.end) {
        spIn.set("start", filter.start);
        spIn.set("end", filter.end);
      } else if (filter?.month && filter?.year) {
        spIn.set("month", filter.month);
        spIn.set("year", String(filter.year));
      }
      return spIn;
    };
            ...
            <thead className="sticky top-0 bg-[#5c8bc1] z-[60]">
              <tr>
                <th
                  ...
                  className={`${baseHeadClass} sticky left-0 top-0 bg-[#5c8bc1] z-[70]`}
                >
                ...
                pageRows.map((row, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      id={`porep-row-${row._id || row.orderNo}`}
                    ...
                    >
                      ...
                      <td className={`${baseCellClass} text-left`}>
                        ${row.totals.overallAfterRefund.toFixed(2)}
                      </td>
                      <td className={`${baseCellClass} text-left`}>
                        <button
                          onClick={(e) => {
