...
const DeliveryReport = () => {
  ...
  const buildBaseParams = (filter = {}) => {
    const params = { limit: ROWS_PER_PAGE };
    if (filter.start && filter.end) {
      ...
      params.month = def.month;
      params.year = String(def.year);
    }
    return params;
  };
  ...
  const processedRows = useMemo(() => {
    return (rawOrders || [])
      ...
      const expeditedInfos = infos.filter(
        (info) =>
          info?.yardExpedite === true || info?.yardExpedite === "true"
      );
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
        <div className="flex items-center gap-6">
          <div>
            <h2 className="text-3xl font-bold text-[#04356d]">Yard Expedite</h2>
