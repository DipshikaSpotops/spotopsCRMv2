...
function normalizePartRequiredLabel(partRequired = "") {
  ...
  if (matches([/\btransmission\b/, /\bused transmission\b/, /\bmanual transmission\b/, /\bautomatic transmission\b/, /\breman/i, /\brebuild/i]) && !lower.includes("control module")) {
    return "Transmission";
  }
  ...
  if (matches([/\binstrumental cluster\b/, /\binstrument cluster\b/, /\bspeedometer\b/])) return "Instrumental Cluster (Speedometer)";
  ...
  const partWiseLeadTotals = useMemo(() => {
    ...
    agentStats.forEach((agent) => {
      (agent?.leads || []).forEach((lead) => {
        ...
        const rawPart = String(lead?.partRequired || "").trim();
